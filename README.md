...
## Optional: Connect With DataGrip

DataGrip is not required to run the project.

If you want to connect DataGrip to PostgreSQL, expose PostgreSQL on a host port.
...
Use these DataGrip connection settings:
...
Port `5433` is recommended because `5432` is often already used by a local PostgreSQL installation.

## Development Mode

For SRE/Terraform checks, use:
...
Run frontend development mode only if you are actively editing the frontend:
